...
dotenv.load_dotenv(dotenv.load_dotenv(dotenv_path="/Users/henrybell/Desktop/Reflect/alignment_research_critique_revision/.env"), override=True)
max_retries = 5
...
def generate_response_with_critique(
    user_message, constitution_path="constitution.json", verbose=False, base_included=False
):
    ...
    elif base_included == True:
        initial_response = "N/A"
        messages = [
            {"role": "system", "content": system_prompt},
        ]
    ...
    try_again = True
    while try_again:
        try:# Get critique and revision
            response = client.chat.completions.create(
                model=model, messages=messages, temperature=temperature, max_tokens=max_tokens
            )
            try_again = False  # Exit loop if successful
