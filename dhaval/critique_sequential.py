...
def build_single_principle_critique_request(principle_name, principle_data):
    """Build a critique request for a single principle."""
    return f"""CritiqueRequest: Please analyze the above response according to this constitutional principle:

{principle_name}:
- Definition: {principle_data['definition']}
- Analysis: Analyze how well the response adheres to or violates this principle.

After analyzing the principle, provide:
- Summary of main issues
- Key violations of the principle
- Suggested improvements

RevisionRequest: Based on the above critique, please rewrite the response to better align with this constitutional principle while maintaining helpfulness and honesty.

Revision: [Your improved response]
"""


def generate_response_with_sequential_critique(
    user_message, constitution_path="constitution.json", verbose=False
):
    """Generate a response with sequential critique-revision cycle.
    ...
    # Sequentially critique and revise for each principle
    for category in ["standard", "weird"]:
        for principle_name, principle_data in constitution.get(category, {}).items():
            if verbose:
                print(f"\nCritiquing for principle: {principle_name}")

            # Build critique request for this principle
            critique_request = build_single_principle_critique_request(
            ...
            critique_and_revision = response.choices[0].message.content
            ...
            # Extract the revised response (assuming it's after "Revision:" in the response)
            if "Revision:" in critique_and_revision:
                revised_response = critique_and_revision.split("Revision:")[-1].strip()
                # Only update current_response if there's actually a new revision
                if (
                    not revised_response.lower().startswith("no revision")
                    and revised_response.strip() != "[Your improved response]"
                ):
                    current_response = revised_response

            if verbose:
                print(f"\nCritique and revision for {principle_name}:")
                print(critique_and_revision)

    return {
        "initial_response": messages[2]["content"],  # Original response
...
result = generate_response_with_sequential_critique(
...
print("\n" + "=" * 80)
print("FINAL RESPONSE AFTER ALL REVISIONS:")
print("=" * 80)
print(result["final_response"])

if result["critique_history"]:
    print("\n" + "=" * 80)
    print("PRINCIPLES APPLIED IN ORDER:")
    print("=" * 80)
    for critique in result["critique_history"]:
        print(f"- {critique['principle']}")
